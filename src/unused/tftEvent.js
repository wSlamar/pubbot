...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        console.log('\x1b[36m', `/mojito-tft has been kicked off by [${interaction.user.username}#${interaction.user.discriminator}] at [${convertTZ(estDateLog, 'America/New_York').toLocaleString()}]`, '\x1b[0m')
        ...
        collector.on("collect", async (reaction, user) => {
            estDateLog = new Date()
            console.log('\x1b[36m', '/mojito-tft:', '\x1b[32m', `Collected [${reaction.emoji.name}] from [${user.tag}] at [${convertTZ(estDateLog, 'America/New_York').toLocaleString()}]`, '\x1b[0m');
                ...
                if (member.permissions.has(PermissionFlagsBits.MuteMembers)) {
                    ...
                    if (countOfEmpty > 0) {
                        ...
                        let reminder = await channel.send({
                            content: `${eventPing} There ${countOfEmpty === 1 ? "is" : "are"} **${countOfEmpty}** ${spotsText} open in the Custom TFT event! Go to <#${mojitoTftChannel}> to sign up! ${tftEmoji}`,
                        });
                    }
                }
            }
        ...
        collector.on("end", (collected) => {
            console.log('\x1b[36m', '/mojito-tft:', '\x1b[31m', `Collected ${collected.size} total emoji reactions`, '\x1b[0m');
        });
        ...
        // This function refreshes the embed for the TFT lobby message.
