...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        console.log('\x1b[36m', `/mojito-arena has been kicked off by [${interaction.user.username}#${interaction.user.discriminator}] at [${convertTZ(estDateLog, 'America/New_York').toLocaleString()}]`, '\x1b[0m')
        ...
        let messageContent = `${eventPing} this Arena event will start <t:${eventDayMomentUnix}:R>`
        ...
        collector.on("collect", async (reaction, user) => {
            estDateLog = new Date()
            console.log('\x1b[36m', '/mojito-arena:', '\x1b[32m', `Collected [${reaction.emoji.name}] from [${user.tag}] at [${convertTZ(estDateLog, 'America/New_York').toLocaleString()}]`, '\x1b[0m');
            ...
            for (let i = 1; i <= 16; i++) {
                if (reaction.emoji.name === playerEmoji && playerMap.get(`bluePlayer${i}`).includes(`[PLAYER ${i} OPEN SPOT]`) && usernameNoTag !== "Mojito" && !valuesArray.includes(true)) {
                    playerMap.set(`bluePlayer${i}`, [`<@${userNameID}>`, userNameID, fullUserName]);
                    checkIDs(userNameID);
                    break;
                }
            }
                ...
                if (member.permissions.has(PermissionFlagsBits.MuteMembers)) {
                    ...
                    if (countOfEmpty > 0) {
                        ...
                        let reminder = await channel.send({
                            content: `${eventPing} There ${countOfEmpty === 1 ? "is" : "are"} **${countOfEmpty}** ${spotsText} open in the Custom Arena event! Go to <#${mojitoArenaChannel}> to sign up! ${arenaEmoji}`,
                        });
                    }
                }
            }
        ...
        collector.on("end", (collected) => {
            console.log('\x1b[36m', '/mojito-arena:', '\x1b[31m', `Collected ${collected.size} total emoji reactions`, '\x1b[0m');
        });
