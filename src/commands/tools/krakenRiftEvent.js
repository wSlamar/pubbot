    ...
    async execute(interaction, client) {
        ...
        buttonCollector.on('collect', async (interaction) => {
            ...
            if (interaction.customId === "signup" && !valuesArray.includes(true)) {
                ...
                if (!roleFound) {
                    await interaction.reply({ content: `⚠️ You do not have one of the required roles to sign up for this event. Head over to <id:customize> to get your League of Legends rank role and then try signing up again.`, ephemeral: true }).catch(error => console.error('An error occurred:', error));
                }
            ...
            if (interaction.customId === "settings" && interaction.member.permissions.has(PermissionFlagsBits.MuteMembers)) {
                ...
                settingsButtonCollector.on('collect', async i => {
                    console.log('\x1b[36m', '/kraken-rift:', '\x1b[32m', `Collected [${i.customId.toUpperCase()}] from [${i.user.username}] at [${convertTZ(estDateLog, 'America/New_York').toLocaleString()}]`, '\x1b[0m');
                    if (i.customId === 'pin') {
            ...
            const countDownFn = async () => {
                try {
                    ...
                    if (error.code === 10008) { // Unknown Message error code
                        console.log('\x1b[36m', '/kraken-rift:', '\x1b[31m', `Message was deleted, stopping interval`, '\x1b[0m');
                        clearInterval(interval);
                        buttonCollector.stop();
                    } else {
                        handleError(error);
